...
const useStoreImpl = create<IState>((set: SetState<IState>, get: GetState<IState>) => {
  ...
  const actions: Actions = {
    ...controlActions,
    ...booleanActions,
    forward: (state) => socket.emit('controls', { forward: state }),
    backward: (state) => socket.emit('controls', { backward: state }),
    left: (state) => socket.emit('controls', { left: state }),
    right: (state) => socket.emit('controls', { right: state }),
    camera: () => set(
      (state) => (
